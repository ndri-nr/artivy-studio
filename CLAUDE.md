...
| `parkeer/`           | **Parkeer!** — Godot 4 parking-jam prototype      | `github.com/ndri-nr/parkeer` (unpushed) |
...
`parkeer/` has its remote set to `github.com/ndri-nr/parkeer` but **nothing has
been pushed and `git ls-remote` cannot reach it**, so `bootstrap.sh` still does
not list it: a line there for a repo that does not answer fails the script for
every repo after it. Add the line once the first push lands, not before. It is a
fifth toolchain in name only — Godot 4.7 like StackO!, but its own project with
no menu, save, ads or signing yet. `parkeer/CLAUDE.md` says what is deliberately
missing.
